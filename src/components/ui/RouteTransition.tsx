  ...
  return (
    <AnimatePresence mode="wait" initial={false}>
      <motion.div
        ...
        exit={reducedMotion ? { opacity: 1 } : { opacity: 0, y: -12, filter: "blur(10px)" }}
        transition={{ duration: reducedMotion ? 0.2 : 0.58, ease: [0.22, 1, 0.36, 1] }}
      >
        <motion.div
          className="route-shutter-overlay"
          initial={reducedMotion ? undefined : { opacity: 0.65, scaleY: 1 }}
          animate={reducedMotion ? undefined : { opacity: 0, scaleY: 0.12 }}
          transition={{ duration: 0.72, ease: [0.22, 1, 0.36, 1] }}
          aria-hidden
        />
        <motion.div
          className="route-flare-overlay"
          initial={reducedMotion ? undefined : { opacity: 0.5, x: "-30%" }}
        ...
        <motion.div
          ...
          initial={reducedMotion ? undefined : { opacity: 0, y: 20, filter: "blur(6px)" }}
          animate={reducedMotion ? undefined : { opacity: [0, 1, 0], y: [18, 0, -14], filter: "blur(0px)" }}
